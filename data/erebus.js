...
client.on("message", message => {
    if(message.content.toLowerCase() == "Erebus") 
    return message.channel.send(`Remember This Name Because This Name Will Fuck Your Life`)
});
...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author},Günaydın hoşgeldin`)
});
...
client.on("message", message => {
    if(message.content.toLowerCase() == "Erebus") 
    return message.channel.send(`${message.author}, Remember This Name Because This Name Will Fuck Your Life`)
});
//------------------------------------------------------------------------------------------------------------\\
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('808587482379583548');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('808220144928161793');
  }, ms(süre));
}
})
...
data.set(`${member.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
  ...
  .setColor(`#0x800d0d`)
  .setDescription(`Çokmu zekisin be kardeşim ama ben senden daha zekiyim unutma ;)`)
  .setTimestamp()
    member.send(wasted)
} 
          ...
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                      ...
                      return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Kardeşim küfür yasak saygılı ol!`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
          ...
          try {
            if (!oldMessage.member.hasPermission("BAN_MEMBERS")) {
                  oldMessage.delete();
                      ...
                      return oldMessage.channel.send(new Discord.MessageEmbed().setDescription(`${oldMessage.author} Kardeşim küfür yasak saygılı ol!`).setColor('0x800d0d').setAuthor(oldMessage.member.displayName, oldMessage.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
 ...
 if(!db.has(`reklam_${msg.guild.id}`)) return;
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Bu sunucuda reklam yasak herhangi bir reklam yapamazsın bunu unutma! Birdaha görmiyim!`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
...
client.on("ready", () => {
    client.channels.cache.get("807170488769970186").join()
    })
